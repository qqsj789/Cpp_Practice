...
使用 istringstream 从字符串中传输整数

在 C++ 中，字符串可以流式传输到临时变量中，
类似于如何将文件流式传输为字符串。
流式传输字符串允许我们单独处理每个字符。

流式传输字符串的一种方法是使用输入字符串流对象
来自 <sstream> 标头的 istringstream。

一旦创建了 istringstream 对象，部分字符串
可以使用“提取运算符”进行流式传输和存储：>>。
提取操作符将一直读取直到到达空格或直到流失败。
执行以下代码以查看其工作原理： 
*/
